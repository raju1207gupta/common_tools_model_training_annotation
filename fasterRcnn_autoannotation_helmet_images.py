...
path  = 'D:/deeplearning learn/Social distancing/face-mask-detector/dataset_HUL_Helmet_annotate/images/'
...
PATH_TO_IMAGE = "D:/deeplearning learn/Social distancing/face-mask-detector/dataset_HUL_Helmet_annotate/images_raw/"
...
for item in dirs:
    if os.path.isfile(PATH_TO_IMAGE+item):
        ...
        cols = img.shape[0]
        if frmnumber>0:
            cv2Net.setInput(cv2.dnn.blobFromImage(img, size=(1080*3,720*3), swapRB=True, crop=False))
            ...
            idxs = cv2.dnn.NMSBoxes(boxes, confidences, 0.55, 1)
            ...
            cv2.imshow('img', img)
            cv2.imwrite(os.path.join(path,f'{str(item)}'),img)
            create_labimg_xml(os.path.join(path,f'{str(item)}'), anotation_list)
